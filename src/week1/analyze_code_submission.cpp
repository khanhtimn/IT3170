    ...
    void process()
    {
        ...
        while (std::getline(std::cin, line) && line != "#") {
            ...
            if (submission.status == "OK") {
                user_problem_max_points[submission.user_id][submission.problem_id] = std::max(user_problem_max_points[submission.user_id][submission.problem_id], submission.point);
            }
        }
        ...
        while (std::getline(std::cin, line) && line != "#") {
            ...
            if (query == "?total_number_submissions") {
                std::cout << totalNumberSubmissions() << std::endl;
            } else if (query == "?number_error_submision") {
                std::cout << numberErrorSubmissions() << std::endl;
            } else if (query == "?number_error_submision_of_user") {
                std::string user_id;
                ss >> user_id;
                std::cout << numberErrorSubmissionsOfUser(user_id) << std::endl;
            } else if (query == "?total_point_of_user") {
                std::string user_id;
                ss >> user_id;
                std::cout << totalPointOfUser(user_id) << std::endl;
            } else if (query == "?number_submission_period") {
                std::string from_time, to_time;
                ss >> from_time >> to_time;
                std::cout << numberSubmissionPeriod(from_time, to_time) << std::endl;
            }
        }
    }
...
private:
    ...
    int totalNumberSubmissions()
    {
        return submissions.size();
    }

    int numberErrorSubmissions()
    {
        return count_if(submissions.begin(), submissions.end(), [](const Submission& s) {
            return s.status == "ERR";
        });
    }

    int numberErrorSubmissionsOfUser(const std::string& user_id)
    {
        return user_error_count[user_id];
    }

    int totalPointOfUser(const std::string& user_id)
    {
        int total_points = 0;
    ...
    int numberSubmissionPeriod(const std::string& from_time, const std::string& to_time)
    {
        auto lower = lower_bound(submissions.begin(), submissions.end(), from_time, [](const Submission& s, const std::string& time) {
            return s.time_point < time;
        });

        auto upper = upper_bound(submissions.begin(), submissions.end(), to_time, [](const std::string& time, const Submission& s) {
            return time < s.time_point;
        });
