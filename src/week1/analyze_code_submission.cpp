...
public:
    ...
    int total() const { return static_cast<int>(submissions_.size()); }

    int totalErrors() const
    {
        return static_cast<int>(std::count_if(
            submissions_.begin(), submissions_.end(),
            [](const Submission& s) { return s.status == "ERR"; }));
    }
    ...
    int countInPeriod(const std::string& from, const std::string& to)
    {
        ...
        auto lo = std::lower_bound(
            submissions_.begin(), submissions_.end(), from,
            [](const Submission& s, const std::string& t) {
                return s.time_point < t;
            });
        auto hi = std::upper_bound(
            submissions_.begin(), submissions_.end(), to,
            [](const std::string& t, const Submission& s) {
                return t < s.time_point;
            });
...
private:
    ...
    std::unordered_map<
        std::string, std::unordered_map<std::string, int>>
        max_points_;
};
